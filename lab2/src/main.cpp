...
void filter_users(const UserCollection& user_collection) {
    ...
    auto users = user_collection.filter_users(email, phone_number);
    if (!users.empty()) {
...
void compare_user_by_name(const UserCollection& user_collection) {
    std::string input_username;
    std::cout << "Enter the username to compare: ";
    std::cin >> input_username;

    User temp_user(-1, input_username, "", "", "");

    bool found = false;
    for (const auto& user : user_collection.filter_users()) {
        if (*user == temp_user) {
            std::cout << "User with username " << input_username
    ...
    if (!found) {
        std::cout << "No user found with username " << input_username << ".\n";
    }
}
...
void handle_choice(int choice, UserCollection& user_collection, sqlite3* db) {
    switch (choice) {
        ...
        case 5:
            ...
            break;
        case 6:
            compare_user_by_name(user_collection);
            break;
        case 7:
...
int main(int argc, char* argv[]) {
    ...
    create_table(db);
    user_collection.set_database(db);
